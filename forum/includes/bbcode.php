...
function extract_search_words ($text, $string_flag = false)
{
	global $bb_cfg, $lang;

	$max_words_count = $bb_cfg['max_search_words_per_post'];
	$min_word_len    = max(2, $bb_cfg['search_min_word_len']);
	...
	{
		if (mb_strlen($word) >= $min_word_len && mb_strlen($word) <= $max_word_len) $output_array[] = $word;
	}
	$text = implode(' ', $output_array);
	unset ($input_array);
	unset ($output_array);
...
#		shuffle($text);
	...
	if ($string_flag) $text = implode(' ', $text);
