...
function post_delete ($mode_or_post_id, $user_id = null, $exclude_first = true)
{
	...
	else
	{
		...
		if ($exclude_first)
		{
			$sql = "SELECT topic_first_post_id FROM ". TOPICS_TABLE ." WHERE topic_first_post_id IN($post_csv)";
	...
	if ($del_user_posts)
	{
		...
		foreach ($db->fetch_rowset("SELECT topic_first_post_id FROM ". TOPICS_TABLE ." WHERE topic_poster IN($user_csv)") as $row)
		{
