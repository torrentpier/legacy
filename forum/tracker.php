	...
	if (!$tr_error && !$tor_list_sql)
	{
	  ...
	  if (SPHINX_ENABLE)
	  {
		...
		if ($title_match)
		{
			...
			$tmp_title_match = extract_search_words($title_match_val, true);
			$tmp_title_match = str_replace(' ', '%', $tmp_title_match);
			if ($tmp_title_match == '') $title_match_notfound_flag = true;
			$SQL['WHERE'][] = "t.topic_title LIKE '%$tmp_title_match%'";
		}
